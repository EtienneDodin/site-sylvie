<x-guest-layout>
    <x-slot name="title">
        Me contacter - Sylvie Buatois, céramiste Raku
    </x-slot>
    ...
    <main>
                ...
                <form id="contact" class="flex flex-col items-center gap-10 px-20 pb-10">
                    ...
                    <div class="flex flex-col items-center gap-2">
                        ...
                        <input type="text" id="name" name="name"
                            class="block w-80 md:w-96 hover:shadow-sm rounded-md border-0 py-1.5 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6"
                            required>
                    </div>
        ...
        {{-- Form send modal --}}
        <div x-data="{ showAlert: false, message: '' }" x-show="showAlert" x-transition x-cloak class="fixed left-0 top-0 w-full h-screen flex justify-center items-center" @custom-alert.window="showAlert = true; message = $event.detail.message; setTimeout(() => showAlert = false, 5000);">
            {{-- Overlay --}}
            <div class="absolute top-0 left-0 w-full h-screen bg-black/20"></div>

            {{-- Modal --}}
            <div @click.outside="showAlert = false" class="bg-white w-3/4 md:w-1/3 z-10 flex flex-col gap-10 items-center px-12 py-14 border border-gray-400 rounded-lg">
                <p x-text="message" class="font-semibold text-center font-anek"></p>

                <button @click="showAlert = false" class="rounded-md py-1.5 w-28 border-2 hover:bg-slate-600 hover:text-gray-200 transition duration-300 ease-in">
                    OK
                </button>
            </div>
        </div>
