<x-app-layout>
    ...
    <div class="flex flex-col gap-10 mt-6 items-center bg-neutral-100">

        {{-- Back and delete --}}
        <div class="flex gap-24 items-center">

            {{-- Back --}}
            <a href="{{ route('creations.index') }}" class="underline underline-offset-4 decoration-gray-300 text-gray-700">Retour à l'accueil</a>
            {{-- Delete creation --}}
            <div>
                <form action="{{ route('creations.destroy', $creation) }}" method="post" x-data="{ open: false }">
                    @method('DELETE')
                    @csrf
                    <button type="button" @click="open = true" class="rounded-md text-gray-100 py-1.5 bg-red-700 hover:bg-red-800 transition duration-200 ease-in-out px-3 text-sm">Supprimer la création</button>
                    ...
                    <div x-show="open" x-transition x-cloak class="fixed left-0 top-0 w-full h-screen flex justify-center items-center">
                        ...
                        <div @click.outside="open = false" class="bg-gray-300 flex flex-col p-24 items-center gap-24 rounded border shadow z-10 w-6/12">
                            ...
                            <div class="flex gap-20">
                                ...
                                <button type="button" @click="open = false" class="rounded-md text-gray-200 py-1.5 border bg-gray-600 hover:bg-gray-500 px-8">Non</button>
                            </div>
                        </div>
                    </div>
            ...
            <form action="{{ route('creations.update', $creation) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="flex flex-col items-center gap-7">
                    ...
                    <div class="flex gap-4 items-center">
                        ...
                        <input type="checkbox" name="sold" id="sold" value="1" {{ $creation->sold ? 'checked' : '' }}>
                    </div>
                    ...
                    <div class="flex gap-4 items-center">
                        ...
                        <input type="checkbox" id="gallerymsg" name="gallerymsg" value="1" {{ $creation->gallerymsg ? 'checked' : '' }}>
                    </div>
                    ...
                    <button type="submit" class="mt-2 rounded-md border py-1.5 px-3 bg-gray-500 hover:bg-gray-400 text-gray-200 hover:text-gray-100 transition duration-200 ease-in-out">Mettre à jour</button>
                </div>
                    ...
                    <div class="flex gap-4">
                        ...
                        @foreach ($creation->images as $image)
                            <div class="flex flex-col gap-6 items-center">
                                {{-- Display image --}}
                                <div class="h-20 overflow-hidden max-w-32">
                                    <img src="{{ $image->path }}" alt="{{ $creation->description }}" class="h-20 object-cover border rounded shadow">
                                </div>
                                ...
                                <form action="{{ route('images.destroy', $image) }}" method="post" x-data="{ open: false }">
                                    @method('DELETE')
                                    @csrf
                                    <button type="button" @click="open = true" class="rounded-md text-gray-200 py-1.5 bg-amber-700 hover:bg-amber-800 px-3">Supprimer</button>
                                    ...
                                    <div x-show="open" x-transition x-cloak class="fixed left-0 top-0 w-full h-full flex justify-center items-center">
                                        ...
                                        <div @click.outside="open = false" class="bg-gray-200 flex flex-col py-24 items-center gap-24 rounded border shadow z-[1] w-1/2">
                                            ...
                                            <div class="flex gap-20">
                                                <button type="submit" class="rounded-md text-gray-200 py-1.5 bg-amber-700 hover:bg-amber-800 px-8">Oui</button>
                ...
                <form action="{{ route('images.store', $creation) }}" method="post" enctype="multipart/form-data" x-data="{ imagePreview: '' }">
                    ...
                    <div class="flex flex-col gap-12 items-center">
                        <label for="image">
                            ...
                            <input type="file" id="image" name="image" class="file:text-amber-800 file:bg-gray-200 hover:file:bg-gray-300 file:px-3 file:py-1.5 file:mr-2 file:border file:border-gray-100 file:rounded text-gray-600" accept="image/*" @change="imagePreview = URL.createObjectURL($event.target.files[0])">
                        </label>

                        <div class="flex flex-col gap-6 items-center">
                            {{-- image preview --}}
                            <img x-show="imagePreview" x-cloak :src="imagePreview" alt="Image Preview" class="rounded border w-28">
                            <button x-show="imagePreview" x-cloak type="submit" class="bg-gray-200 hover:bg-gray-300 border flex gap-2 items-center border-gray-400 text-gray-800 text-sm font-semibold rounded px-4 py-2">
